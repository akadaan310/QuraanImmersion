/**
 * api/ingest.js — بحر الساعات العليا: the deployed upper-sea clock service.
 *
 * Deployed at the app's own origin, so the browser syncs same-origin with no
 * CORS preflight on every frame. This is what makes the upper sea genuinely
 * distributed rather than a second local array: its proper times make a round
 * trip through the cloud, and the round trip's own latency is reported back and
 * folded into the sea's state.
 *
...
import { decodeFrame, summarizeLogTau } from './_frame.js';
...
export default async function handler(req, res) {
  ...
  return res.status(200).json({
    ...
    dedupe: 'warm',
    protocol: 1,
    // Server clock, so the browser can measure the round trip and expose the
    // upper sea's true distance rather than asserting one.
    serverTime: Date.now(),
